...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private static final String PATH = Environment.getExternalStorageDirectory().getPath() + "/netfon_crash/";
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        try {
            ...
            uploadExceptionToServer(ex.toString());
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        //如果系统提供默认的异常处理程序，它将被交给系统结束我们的程序，否则我们将自己结束。
    ...
    private void dumpExceptionToSDCard(Throwable ex) throws IOException {
        ...
        //Create a log file at the current time
        File file = new File(PATH + FILE_NAME + time + FILE_NAME_SUFFIX);

        try {
            PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(file)));
            //The time of occurrence of an exception is derived.
            pw.println(time);
    ...
    private void uploadExceptionToServer(String exceptions) {
    }
}